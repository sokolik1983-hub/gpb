...
import { SCROLLER, TABLE, TABLE_HEADER } from 'stream-constants/a11y';
...
export const DataTable = function Table<T extends { id: string }>({
  ...
  return (
      <FocusNode nodeId={TABLE} parentId={SCROLLER}>
      <Box className={css.wrapper}>
        <Box {...getTableProps()}>
          {rows.length > 0 &&
            headerGroups.map(headerGroup => (
              // eslint-disable-next-line react/jsx-key
                <FocusNode nodeId={TABLE_HEADER} parentId={TABLE}>
                  ...
                  <WithClickable key={row.getRowProps().key}>
                    {(ref, { hovered }) => (
                      <React.Fragment key={`rowLayout_${row.getRowProps().key}`}>
                        <FocusNode nodeId={row.id} parentId={TABLE}>
                        <Box
                          ref={ref}
                          border={['FAINT', 'SM']}
