...
export const GroupingRow: FC<IGroupingRowProps> = ({ groupingRow }) => {
  ...
  if (groupingType === GROUPING_TYPE.CURRENCIES) {
    return (
      <WithClickable>
        {(ref, { hovered }) => (
          <Box {...rowProps} ref={ref} className={cn(css.borderedRow, css.subGroupRows, { [css.hoveredRow]: hovered })}>
            {cells.map(cell => {
  ...
  return (
    <WithClickable>
      {(ref, { hovered }) => (
        <Box {...rowProps} ref={ref} className={cn(css.groupRows, { [css.borderedRow]: !isExpanded, [css.hoveredRow]: hovered })}>
          <Horizon>
